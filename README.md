...
### Resources File
The installation creates a file named ResourcesPaths.ini at the <resources dir> root (set with *configure.sh*) which specifies the default path to the required programs and data files (such as genomes and tables). **Modify this file after installtion to change defaults (such as in the case of not downloading the data files)**
