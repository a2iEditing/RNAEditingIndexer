...
then
  ...
  echo "Done Processing MM10 RefSeq Table ${MM10_ALL_REFSEQ_TABLE_FILE}"
  ...
  echo "moving Murine Gene Expression Files from ${DEV_ROOT}/make/GenesExpressionMusMusculus/ to ${MURINE_GENES_EXPRESSION_DIR}/"
  mv ${DEV_ROOT}/make/GenesExpressionMusMusculus/* "${MURINE_GENES_EXPRESSION_DIR}/"
  # commented out - we use localy files instead of downlioading and processing again
  ...
  then
    ...
    for table in ${MM9_REGIONS_TABLES[*]}
    do
    # the alternative to cat multiple files
    cat ${MURINE_REGIONS_DIR}/${table} >> "${MM9_REGIONS_TABLE_COMBINED}"
